...
// Copyright (c) 2017-2018 The Bifrost developers
...
void OverviewPage::updateObfuscationProgress()
{
    ...
    QString strAnonymizeBifrostAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeBifrostAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if (currentBalance == 0) {
        ...
        strAnonymizeBifrostAmount = strAnonymizeBifrostAmount.remove(strAnonymizeBifrostAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeBifrostAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    ...
    if (nMaxToAnonymize > nAnonymizeBifrostAmount * COIN) nMaxToAnonymize = nAnonymizeBifrostAmount * COIN;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= nAnonymizeBifrostAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                              .arg(strAnonymizeBifrostAmount));
        strAnonymizeBifrostAmount = strAnonymizeBifrostAmount.remove(strAnonymizeBifrostAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeBifrostAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    } else {
